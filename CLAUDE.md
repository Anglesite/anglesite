...
**Issue-in-flight signaling.** Multiple agents work this repo concurrently (see "Worktrees" above), so before starting work on a tracked issue, check it isn't already claimed and mark that you're taking it: `gh issue list --label "🛠️ In Progress"` to check, then `gh issue edit <n> --add-label "🛠️ In Progress"` to claim it. Only remove the label (`gh issue edit <n> --remove-label "🛠️ In Progress"`) once the PR is open **and its body actually links the issue with a closing keyword** (`Closes #<n>` / `Fixes #<n>` — see `CONTRIBUTING.md` ▸ "Commits and pull requests"); a PR that only mentions the issue number in prose doesn't link it, and removing the label anyway leaves the issue looking unclaimed. If you find an issue already fixed/merged before you could start (as happens — two agents can pick the same issue in the same window), don't silently redo the work: check for an existing PR/commit first, and if one already landed, close the issue referencing it rather than duplicating the fix.
