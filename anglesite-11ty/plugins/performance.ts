 ...
 * @param logger - Optional logger instance for environment-aware logging
 */
function checkPerformanceBudget(
  ...
  if (totalCSSSize > (thresholds.maxCSSSize || 100) * 1024) {
    const message = `Performance Budget Warning: CSS size (${Math.round(totalCSSSize / 1024)}KB) exceeds threshold (${thresholds.maxCSSSize || 100}KB)`;
    if (logger) {
      logger.warn(message);
    } else {
      console.warn(`[@dwk/anglesite-11ty] ${message}`);
    }
  }
  ...
  if (totalJSSize > (thresholds.maxJSSize || 200) * 1024) {
    const message = `Performance Budget Warning: JS size (${Math.round(totalJSSize / 1024)}KB) exceeds threshold (${thresholds.maxJSSize || 200}KB)`;
    if (logger) {
      logger.warn(message);
    } else {
      console.warn(`[@dwk/anglesite-11ty] ${message}`);
  ...
  if (images.length > (thresholds.maxImages || 50)) {
    const message = `Performance Budget Warning: Image count (${images.length}) exceeds threshold (${thresholds.maxImages || 50})`;
    if (logger) {
      logger.warn(message);
    } else {
      console.warn(`[@dwk/anglesite-11ty] ${message}`);
...
export default function addPerformanceOptimization(
  ...
  const logger = createPluginLogger('performance');
  ...
  eleventyConfig.addTransform('performance-optimization', function (...args: unknown[]) {
    ...
    try {
      ...
      if (mergedOptions.performanceBudget) {
        checkPerformanceBudget(optimizedContent, mergedOptions.outputDir, mergedOptions.budgetThresholds, logger);
      }
  ...
  eleventyConfig.addShortcode('inlineCSS', function (...args: unknown[]) {
    ...
    try {
      ...
      if (existsSync(fullPath)) {
        ...
        logger.warn(`CSS file not found for inlining: ${fullPath}`);
  ...
  eleventyConfig.addShortcode('inlineJS', function (...args: unknown[]) {
    ...
    try {
      ...
      if (existsSync(fullPath)) {
        ...
        logger.warn(`JS file not found for inlining: ${fullPath}`);
