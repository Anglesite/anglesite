...
Applied when building a contact form during `/start` or `/design-interview`. Every business site needs a contact form — build it secure from the start.
...
- During `/check`, verify that images in `public/images/` don't contain GPS EXIF data.
...
The scaffold uses system fonts by default — zero external requests. If the owner wants custom fonts during `/design-interview`:
...
- **Airtable** (if `/setup-customers` is used): customer data lives on Airtable's servers (US-based). Disclose in privacy policy if used. Note for EU-audience businesses: Airtable processes data in the US.
...
Share with the owner during `/deploy` (Cloudflare account creation) and `/domain` (DNS setup). Keep it brief and non-technical.
...
What to do when things go wrong. Reference during `/fix` or when the owner reports a problem.
...
- `npm audit` runs during `/check`. Fix vulnerabilities before deploying.
- `package-lock.json` must be committed — it pins exact dependency versions and prevents supply chain substitution attacks.
- `/update` command handles quarterly dependency updates per the maintenance schedule in `webmaster.md`.
...
| PII scan | `.claude/commands/deploy.md` | Blocks deploy if email addresses or phone numbers found in built HTML |
| Token scan | `.claude/commands/deploy.md` | Blocks deploy if API tokens (Airtable `pat*`, Stripe `sk-*`) found in source or build |
| Third-party script scan | `.claude/commands/deploy.md` | Blocks deploy if unauthorized external `<script>` tags found |
| Keystatic leak check | `.claude/commands/deploy.md` | Blocks deploy if `/keystatic` admin routes appear in production build |
| Health checks | `.claude/commands/check.md` | Validates headers, runs npm audit, checks privacy, verifies accessibility |
