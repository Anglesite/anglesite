...
Legal requirements and best practices that apply to every business website. The agent applies these during `/design-interview` and `/start`, on top of the business type's Compliance section.
...
3. During `/design-interview` or `/start` (Step 2), ask the owner about applicable items — or just add them when no question is needed (copyright notice, accessibility statement).
4. Add the relevant pages, footer links, and notices to the site.
5. During `/check`, verify that privacy policy, copyright notice, and accessibility statement are present.
...
**Ask the owner:** Nothing for the basic statement — just add it. During `/design-interview` (Step 9, accessibility), ask about audience-specific accessibility needs.

Note: WCAG AA compliance is already enforced by `/check` and `/deploy`. The statement is about communication, not a substitute for actual accessibility.
...
**Where to add:** Not a dedicated page. Mention in gallery captions ("Photo by [name]" or "Used with permission") and attribute testimonials by name (or "— Verified customer" if anonymous). Remind the owner during the `/design-interview` gallery and testimonials discussion.
...
**Important:** The `/deploy` security gate already blocks unauthorized third-party scripts. If a third-party embed is approved, update the privacy policy to mention it.
...
- **PII scanning, token scanning, third-party script blocking** → `/deploy` security gates
- **WCAG AA accessibility testing** → `/check` command (already enforced)
