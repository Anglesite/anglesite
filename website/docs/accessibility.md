...
Practical guidance for keeping the site accessible as the owner adds content over time. Reference for the webmaster agent — read during `/design-interview`, content creation, and `/check`. Not user-facing documentation.

For the legal requirement and accessibility statement, see `docs/smb/legal-checklist.md`. For automated testing, see `.claude/commands/check.md`. This file covers the *why* and *how* of accessible content — the things that can't be caught by a linter alone.
...
- Logical heading hierarchy enforced by `/check`
...
**During `/design-interview`:** When the owner provides photos, write the alt text for them. Don't ask "what should the alt text be?" — write a good description and confirm. Most people don't know what alt text is.
...
### What `/check` does automatically

- `pa11y` scans for WCAG 2.1 AA violations (contrast, alt text, ARIA, headings, labels)
- Manual checklist in `/check` covers heading hierarchy, skip link, keyboard navigation, images of text, lang attribute
...
Keep it simple. During `/design-interview` or when delivering the site:
