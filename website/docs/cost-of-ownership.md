...
What the website costs to run, now and ongoing. Reference for the webmaster agent — shared with the owner during `/start` (Step 7) and `/deploy`, and revisited during annual maintenance. Not user-facing documentation.

## The basics (already covered in `/start`)
...
This is a differentiator worth mentioning during `/start`:
...
- **No maintenance contract** — `/check` and `/update` handle maintenance.
