...
Practical design reference for the webmaster agent. Read during `/design-interview` and `/start`, and when creating or modifying pages. Not user-facing documentation.
...
These are design decisions, not code fixes. Apply during `/design-interview`:
