...
During `/start` Step 2 (design interview), when asking about social profiles. Also during `/domain` when setting up Bluesky verification.
...
- **Bluesky** — Use domain as handle (see `/domain` → Bluesky verification)
...
Optional properties to add during `/design-interview` if the business has a physical location:
...
Don't recommend it during `/start`. It adds complexity that most SMB owners don't need initially.
...
### During `/start` and `/design-interview`
