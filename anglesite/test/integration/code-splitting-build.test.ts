...
describe('Code Splitting Build Integration', () => {
  ...
  describeOrSkip('Production Build Output', () => {
    ...
    it('should have content-based hashing in filenames', () => {
      ...
      jsFiles.forEach(file => {
        if (!file.startsWith('runtime.')) { // Runtime might not have hash
  ...
  describeOrSkip('Bundle Analysis Output', () => {
    ...
    it('should show correct chunk dependencies in stats', () => {
      ...
      // Find forms chunk
      const formsChunk = stats.chunks?.find((c: any) => 
        c.names?.includes('forms') || c.id === 'forms'
      );
      
      if (formsChunk) {
        // Forms chunk should contain @rjsf modules
        const formsModules = stats.modules?.filter((m: any) => 
          formsChunk.modules?.includes(m.id) || 
          formsChunk.containedModules?.includes(m.id)
        );
        
        const hasRjsfModules = formsModules?.some((m: any) => 
          m.name?.includes('@rjsf') || m.identifier?.includes('@rjsf')
        );
  ...
  describe('Error Boundary Component', () => {
    it('should export LazyComponentErrorBoundary class', () => {
      const mainPath = path.join(
        projectRoot,
        'app/ui/react/components/Main.tsx'
      );
  ...
  describe('Webpack Magic Comments', () => {
    it('should use webpackChunkName for lazy imports', () => {
      ...
      expect(mainContent).toMatch(
        /import\s*\(\s*\/\*\s*webpackChunkName:\s*["']website-config-editor["']\s*\*\//
      );
    });
  });
});