...
export class WebsiteService {
  ...
  /**
   * Example 2: Using the @HandleErrors decorator.
   */
  @HandleErrors<unknown[]>(async (error: AngleError) => {
    // Recovery strategy - return empty array on error
    console.warn(`Failed to get websites: ${error.message}`);
    return [];
  })
  async getWebsites(): Promise<unknown[]> {
    // This method will automatically handle errors and apply recovery
    ...
    return await withContext(
      {
        ...
        website: { id: undefined, name, domain },
...
export class EnhancedWebsiteManager {
  async createWebsite(config: Record<string, unknown>): Promise<unknown> {
    return await withContext(
      {
        operation: 'createWebsite',
        website: { id: undefined, name: config.name, domain: config.domain },
      },
      async () => {
        try {
