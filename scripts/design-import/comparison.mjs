...
async function main() {
  const [sourceBaseUrl, localBaseUrl, ...pagePaths] = process.argv.slice(2);

  if (!sourceBaseUrl || !localBaseUrl) {
    console.error(
      'Usage: node comparison.mjs <source-base-url> <local-base-url> [page-paths...]',
    );
    console.error('Example: node comparison.mjs https://example.my.canva.site http://localhost:4321 / /services');
    process.exitCode = 1;
    return;
  }

  const paths = pagePaths.length > 0 ? pagePaths : ['/'];
  await captureComparisons(sourceBaseUrl, localBaseUrl, paths);
  for (const pagePath of paths) {
    const { original, generated } = comparisonPaths(pagePath);
    console.log(`${pagePath} → ${original} | ${generated}`);
  }
}
