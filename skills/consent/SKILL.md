...
| Default | When |
|---|---|
| **Geo (recommended)** | Default-deny in EU/UK; default-allow elsewhere. Requires the Cloudflare Pages middleware below. |
...
If either is missing, copy from `${CLAUDE_PLUGIN_ROOT}/template/src/scripts/consent.ts` and `${CLAUDE_PLUGIN_ROOT}/template/src/components/ConsentBanner.astro` to the user's site.
...
If `src/pages/privacy.astro` does not exist yet, copy `${CLAUDE_PLUGIN_ROOT}/template/src/pages/privacy.astro` to the site.
...
- A Cloudflare Pages Function detects EU visitors so first-visit defaults match local law (only if you picked the geo policy).
