...
- [ADR-0003 Cloudflare Workers](${CLAUDE_PLUGIN_ROOT}/docs/decisions/0003-cloudflare-workers-hosting.md) — Cloudflare hosting and Workers
- [ADR-0008 No third-party JS](${CLAUDE_PLUGIN_ROOT}/docs/decisions/0008-no-third-party-javascript.md) — Turnstile is the only accepted third-party script
...
The templates live at `${CLAUDE_PLUGIN_ROOT}/skills/forms/templates/<slug>.json`. Each is a JSON schema describing the form definition you'll write into the Keystatic `forms` collection.
...
1. Copy the chosen template from `${CLAUDE_PLUGIN_ROOT}/skills/forms/templates/<slug>.json` into a new `src/content/forms/<slug>.mdoc` file as Keystatic-compatible frontmatter — or —
