...
The website needs a hosting platform that is free (or near-free) for content-driven sites, provides global CDN distribution, automatic HTTPS, and can deploy automatically from a GitHub repository. The platform should also offer DNS, domain registration, email routing, and analytics so the owner can manage everything in one place.

## Decision Drivers

* Free hosting with no traffic limits for static sites
* Global CDN with automatic HTTPS
* Git integration — push to `main` auto-deploys, preview branches for staging
...
Chosen option: "Cloudflare Pages", because it provides free static hosting with a global CDN, Git integration that auto-deploys from `main` with preview branches, at-cost domain registration, privacy-first analytics, email routing, and Workers — all under one account the owner controls.

### Consequences

* Good, because hosting is genuinely free with no bandwidth caps for static sites
* Good, because Git integration auto-deploys on push to `main` and creates preview deploys for other branches
...
* Good, because preview deployments are automatic for non-production branches (e.g., `draft.PROJECT.pages.dev`)

### Confirmation

First deploy connects the GitHub repo to Cloudflare Pages via the dashboard. Subsequent deploys push to `main` to trigger auto-deploy. Preview deploys are created by pushing to `draft` or other branches.
...
* Good, because Git integration auto-deploys from `main` with preview branches
