...
struct AcknowledgmentsViewModelTests {
    ...
    func loadAllPopulatesEverySource() async {
    ...
    func filteredMatchesCaseInsensitiveSubstring() async {
        let catalogs = makeCatalogs()
        let model = AcknowledgmentsViewModel(load: { catalogs[$0] ?? [] }, log: { _ in })
        await model.loadAll()
    ...
    func emptySearchReturnsEverythingForSource() async {
        let catalogs = makeCatalogs()
        let model = AcknowledgmentsViewModel(load: { catalogs[$0] ?? [] }, log: { _ in })
        await model.loadAll()
    ...
    func loadFailureMarksSourceUnavailableWithoutLosingOthers() async {
        struct Boom: Error {}
        let catalogs = makeCatalogs()
    ...
    func attributionByIDFindsAcrossSources() async {
        let catalogs = makeCatalogs()
        let model = AcknowledgmentsViewModel(load: { catalogs[$0] ?? [] }, log: { _ in })
        await model.loadAll()
    ...
    func attributionByIDDisambiguatesAcrossSources() async {
